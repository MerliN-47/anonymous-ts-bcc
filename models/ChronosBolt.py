...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # Safety guard against silent default baseline fallback (moe2)
        if hasattr(self, 'augment_mode') and self.augment_mode == 'moe2':
            raise RuntimeError(
                "CRITICAL BYPASS GUARD: --augment_mode defaults to 'moe2', "
                "which silently bypasses neighbor fusion in the forward pass. "
                "You must explicitly pass '--augment_mode moe' in your launcher script."
            )
...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str, align: str = "none"):
        assert hasattr(config, "chronos_config"), "Not a Chronos config file"
    ...
    def forward(
        ...
        # scaling
        context, loc_scale = self.instance_norm(context)
        if retrieved_seq is not None and self.augment != 'none':
            ...
            if getattr(self, 'align', 'none') == 'affine':
                ctx_past = context[:, -retrieved_x.shape[-1]:].unsqueeze(1)
                mean_rx = retrieved_x.mean(dim=-1, keepdim=True)
                mean_ctx = ctx_past.mean(dim=-1, keepdim=True)
                var_rx = ((retrieved_x - mean_rx) ** 2).mean(dim=-1, keepdim=True) + 1e-6
                cov = ((retrieved_x - mean_rx) * (ctx_past - mean_ctx)).mean(dim=-1, keepdim=True)
                a = cov / var_rx
                b = mean_ctx - a * mean_rx
                retrieved_y = a * retrieved_y + b
        ...
        if self.augment == 'moe' and retrieved_seq is not None:
        ...
        if self.augment == 'gate' and retrieved_seq is not None:
            # Step 1
